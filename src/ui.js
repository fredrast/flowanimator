/**
 * @file src/ui.js
 * @description Defines the {@link Ui} class containing the functionality for rendering
...
/**
 * @constructor Ui
 * @description Constructor for the ui object containing the functionality for
 * rendering the user interface and processing input from the use
 * @param timeline The animation timeline object, which is shared between the
 * animation and ui objects
 */
export function Ui(timeline) {
  ...
  this.slotToYCoord = slot => {
    return (
      COLUMN_LABELS_Y - 3 * MARGIN - slot * TOKEN_HEIGHT - TOKEN_HEIGHT / 2
    );
  };
  ...
  const sliderLineClick = e => {
    ...
    timeline.setTime(animationMoment);
    sliderButton.cx(x);
  };
  ...
  const sliderButtonDragActive = e => {
    ...
    timeline.setTime(Math.round(progress * this.factor));
